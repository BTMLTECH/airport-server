...
app.post("/api/feedback", async (req, res) => {
    try {
        ...
        // Departure
        protocolOfficerMeet, immigrationAssistance, meetInOrOutside, } = req.body;
        const emailData = {
            serviceType,
...
app.post("/api/customer", async (req, res) => {
    try {
        const { passengerName, contact, email, protocolOfficer, badgeVerification, checkInIssues, checkInComment, } = req.body;
        const emailData = {
            passengerName,
            contact,
            email,
...
app.post("/api/booking", async (req, res) => {
    try {
        ...
        const { fullName, email, phone, services, selectedServicesDetails, flightDate, flightTime, flightNumber, departureCity, arrivalCity, passengers, specialRequests, discountCode, referralSource, totalPrice, currency, type, } = parsedData;
        if (!email || !fullName) {
            return res
                .status(400)
...
app.get("/api/payment/callback", async (req, res) => {
    try {
        ...
        const payment = await Payment_1.Payment.findOne({ reference });
        if (!payment) {
            return res.redirect(`${FRONTEND_URL}/payment/failed`);
        }
        if (data.status === "success") {
            ...
            await payment.save();
            // Send admin email
            await (0, emailUtil_1.sendEmail)(process.env.ADMIN_EMAIL, "New Booking - BTM", "formSubmission.ejs", { payment } // now passing the Payment document directly
            );
            // Send user confirmation email
            await (0, emailUtil_1.sendEmail)(payment.email, "Your Booking Confirmation - BTM", "userConfirmation.ejs", { payment });
